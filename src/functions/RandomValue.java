...
/**
 * RandomValue is a class with methods for producing random deviates
...
public class RandomValue {
    Random rnum;
    /** Creates a new instance of RandomValue */
    public RandomValue() {
        rnum=new Random();
     ...
     * @param p chance of a head on a single flip
    ...
    {
        int [] ans=new int[n];
        for(int i=0;i<n;i++) ans[i]=a+rnum.nextInt((b-a)+1); //(0,....,b-a)
        return ans;
    }
    /**
     ...
     * values variables) but returned as an array of doubles.
     */
    ...
    public double [] Gaussian(double mean, double sd, int n) {
        double [] ans=new double[n];
        for(int i=0;i<n;i++) ans[i]=mean+sd*rnum.nextGaussian();
        return ans;
    }
    /**
    ...
    public double [] Uniform(double a,double b,int n) {
        double [] ans=new double[n];
        for(int i=0;i<n;i++) ans[i]=a+(b-a)*rnum.nextDouble();
        return ans;
    }
    /**
    ...
    public double [] Gamma(int a, int n) {
        ...
        if(a<1){System.out.println("RandomValue.Gamma(): bad value of a  = "+a); return null;} else if(a<10) {
            for(int i=0;i<n;i++) {
                x=1.0;
                for(int j=1;j<=a;j++) x *= rnum.nextDouble();
                ans[i]= - Math.log(x);
            }
        } else{
            for(int i=0;i<n;i++) {
    ...
    public int [] pickKfromM(int K,int m)
    {
        ...
        int []ans=new int[K];
        for(int j=0;j<K;j++)ans[j]=X[m-1-j];
        return ans;
    ...
    public static void main(String [] args) {
        ...
        //System.out.println("Gamma(2): mean="+results[0]+" sd="+results[1]+" Min/max ="+results[2]+"/"+results[3]);
          x=rv.Poisson(1,100);
        results=rv.DoStats(x);
        System.out.println("Poisson(5): mean="+results[0]+" sd="+results[1]+" Min/max ="+results[2]+"/"+results[3]);
